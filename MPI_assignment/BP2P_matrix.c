...
int main (int argc, char **argv)
{
    ...
    if(procid == 0)
    {
        ...
        {
            MPI_Send(A+(job_load*i*An),job_load*An,MPI_DOUBLE,i,0,MPI_COMM_WORLD);
            MPI_Send(B,Bm*Bn,MPI_DOUBLE,i,0,MPI_COMM_WORLD);
        }
        //take care of the last procces corner case
        MPI_Send(A+(job_load*(numprocs-1)*An),remain_job*An,MPI_DOUBLE,numprocs-1,0,MPI_COMM_WORLD);
        MPI_Send(B,Bm*Bn,MPI_DOUBLE,numprocs-1,0,MPI_COMM_WORLD);
    ...
    else
    {
        ...
        {
            MPI_Recv(A_comm+(job_load*procid*An),job_load*An,MPI_DOUBLE,0,0,MPI_COMM_WORLD,NULL);
            ...
            MPI_Send(C_comm + (job_load*procid*Cn), job_load*Cn, MPI_DOUBLE, 0, 0,MPI_COMM_WORLD);
        }
        else
        {
            ...
            MPI_Send(C_comm + (job_load*procid*Cn),remain_job*Cn,MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
        }
    }
    free(A_comm);
    free(B_comm);
